...
Reusable bash libraries live in `shared/lib/` and are **vendored** (copied) into each plugin's `lib/` directory. Plugins reference them via `${CLAUDE_PLUGIN_ROOT}/lib/<lib>.sh`.

> **Workaround note:** Libraries are vendored rather than symlinked due to
> [anthropics/claude-code#53948](https://github.com/anthropics/claude-code/issues/53948),
> which causes plugin install to drop symlinks under `plugins/<name>/lib/`. When that upstream
> issue is resolved, this section and the "Adding a New Shared Library" instructions below should
> revert to the symlink pattern. See [#474](https://github.com/nsheaps/ai-mktpl/issues/474).
...
3. For each plugin that needs it, copy the file and prepend the 3-line vendoring header:
   ```bash
   mkdir -p plugins/my-plugin/lib
   {
     printf '# ⚠️  VENDORED FROM shared/lib/my-lib.sh\n'
     printf '# Do not edit this file directly — modify shared/lib/my-lib.sh and run scripts/sync-shared-lib.sh\n'
     printf '# See https://github.com/nsheaps/ai-mktpl/issues/474 + https://github.com/anthropics/claude-code/issues/53948 for context\n'
     cat shared/lib/my-lib.sh
   } > plugins/my-plugin/lib/my-lib.sh
   ```
4. Run `bash scripts/sync-shared-lib.sh` to verify the copy is in sync
5. Document it in this file

> **Do NOT use `ln -s`** — symlinks are dropped by plugin installation
> ([anthropics/claude-code#53948](https://github.com/anthropics/claude-code/issues/53948)).
> CI will reject PRs that introduce symlinks under `plugins/*/lib/`.
...
- Vendored copies carry a 3-line `VENDORED FROM` header; CI rejects PRs without it or with symlinks
...
- When `hook-logging.sh` is vendored, `log.sh` must also be vendored (it's a dependency)
