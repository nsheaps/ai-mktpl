...
> **Note:** Libraries are vendored (copied) rather than symlinked due to
> [anthropics/claude-code#53948](https://github.com/anthropics/claude-code/issues/53948).
> Do NOT use `ln -s` — CI will reject PRs containing symlinks under `plugins/*/lib/`.
> When the upstream issue is resolved, revert to the symlink pattern and remove this note.
> See [#474](https://github.com/nsheaps/ai-mktpl/issues/474).

1. Copy the library into the plugin's `lib/` directory with the 3-line vendoring header:

   ```bash
   mkdir -p plugins/my-plugin/lib
   {
     printf '# ⚠️  VENDORED FROM shared/lib/log.sh\n'
     printf '# Do not edit this file directly — modify shared/lib/log.sh and run scripts/sync-shared-lib.sh\n'
     printf '# See https://github.com/nsheaps/ai-mktpl/issues/474 + https://github.com/anthropics/claude-code/issues/53948 for context\n'
     cat shared/lib/log.sh
   } > plugins/my-plugin/lib/log.sh
   ```

2. Verify the copy is in sync:

   ```bash
   bash scripts/sync-shared-lib.sh --check
   ```

3. Source it in your script:

   ```bash
   source "${CLAUDE_PLUGIN_ROOT}/lib/log.sh"
   ```

4. If using `hook-logging.sh`, also vendor `log.sh` (it's a dependency) using the same pattern.
