...
client.on("interactionCreate", async (interaction: Interaction) => {
  ...
  if (behavior === "more") {
    ...
    if (!details) {
      await interaction
        .reply({ content: "Details no longer available.", ephemeral: true })
        .catch(() => {});
      return;
    }
...
client.on("messageCreate", (msg) => {
  ...
  // Ignore other bots unless DISCORD_ALLOW_BOTS is enabled
  if (msg.author.bot && process.env.DISCORD_ALLOW_BOTS !== "true") return;
