...
#
# Creates a symlink at .claude/rules/common-sense-beta pointing to this plugin's
...
setup_symlink() {
  ...
  local ai_mktpl_pattern="ai-mktpl/plugins/common-sense-beta/"
  for entry in "$target_dir"/*; do
    ...
    if [ -L "$entry" ]; then
      ...
      if [[ "$resolved" == *"${ai_mktpl_pattern}"* ]]; then
        echo "common-sense-beta: removing stale symlink ${entry} -> ${resolved}" >&2
        rm -f "$entry"
      fi
    fi
  done
  ...
  if [ -L "$link_path" ]; then
    ...
    echo "common-sense-beta: WARNING: ${link_path} is a real directory, not replacing" >&2
    return 0
  fi

  ln -s "$PLUGIN_RULES_DIR" "$link_path"
  echo "common-sense-beta: linked ${link_path} -> ${PLUGIN_RULES_DIR}" >&2
}
...
add_plugin_to_repo() {
  ...
  if [ -f "$target_file" ]; then
    local current
    current="$(jq -r '.enabledPlugins["common-sense-beta@nsheaps-claude-plugins"] // empty' "$target_file" 2>/dev/null || true)"
  ...
  safe_write_settings '.enabledPlugins["common-sense-beta@nsheaps-claude-plugins"] = true'
  echo "common-sense-beta: enabled plugin in ${target_file}" >&2
}
