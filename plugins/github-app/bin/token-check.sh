...
# Update the runtime env file with the refreshed token.
#
# IMPORTANT: This function ONLY rewrites ENV_RUNTIME_FILE (token vars).
# It must NEVER touch the static env file (~/.config/agent/github-app-env-static),
# which holds PATH additions, aliases, and git identity vars written by the
# SessionStart init hook. Overwriting those would break wrapper scripts after
# the first token refresh (~55 min into a session).
update_runtime_env() {
  local token="$1"
