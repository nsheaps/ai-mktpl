...
if [[ -n "${GITHUB_APP_PRIVATE_KEY:-}" && -z "$GITHUB_APP_PRIVATE_KEY_PATH" ]]; then
  ...
  KEY_DIR="${HOME}/.agents/${AGENT_NAME:-.default}/.config"
  mkdir -p "$KEY_DIR"
...
TOKEN_FILE="${GITHUB_TOKEN_FILE:-$(plugin_get_config "tokenFile" "$HOME/.agents/${AGENT_NAME:-.default}/.config/github-token")}"
...
ENV_RUNTIME_FILE="${HOME}/.agents/${AGENT_NAME:-.default}/.config/github-app-env"
