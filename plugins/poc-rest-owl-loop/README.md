...
```
/poc-rest-owl-loop build a project management tool like Linear
```

### Direct Prompt

Just describe your idea in conversation. The `poc-rest-owl-loop` skill activates when it detects a brief project description.

## Skills

| Skill                  | Purpose                                              |
| ---------------------- | ---------------------------------------------------- |
| `poc-rest-owl-loop`    | Main orchestrator — coordinates all phases           |
...
Settings in `poc-rest-owl-loop.settings.yaml` or override in `.claude/plugins.settings.yaml`:
