...
Reproduce Claude Code's built-in `/insights` **without** the built-in tooling:
scan your local session transcripts, classify each session across a fixed facet
taxonomy, write seven narrative sections, and render a single shareable HTML
report with charts and copyable prompts.
