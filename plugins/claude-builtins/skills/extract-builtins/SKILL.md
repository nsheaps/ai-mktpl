...
5. **Stamp per asset; never make a blanket version claim.** The stamp belongs
   ...
   `grep -rnE 'v[0-9]+\.[0-9]+\.[0-9]+' <plugin>/` is the authoritative
   inventory — but only if every stamp is actually written in that form.
   Always write the leading `v` (`- claude version: **v2.1.223**`, not
   `**2.1.223**`); the `v` is what separates a binary stamp from a plugin
   version or an unrelated number, and a stamp missing it is invisible to the
   inventory grep and silently breaks the "per-asset stamps are all correct"
   claim for exactly the file that omitted it.
