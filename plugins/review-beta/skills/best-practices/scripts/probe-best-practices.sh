...
TOOLS=$(
	cat <<'TOOLTABLE'
bp-go-idiom	golangci-lint	major	[ -f go.mod ] && golangci-lint run --out-format=line-number 2>/dev/null | grep -E '^[^ ]+\.go:[0-9]+' || true
...
bp-js-idiom	biome	major	biome lint --reporter=github . 2>/dev/null | grep -E '^[^ ]+:[0-9]+' || true
...
bp-swift-idiom	swiftlint	major	swiftlint lint --quiet --reporter emoji 2>/dev/null | grep -E '^[^ ]+\.swift:[0-9]+' || true
