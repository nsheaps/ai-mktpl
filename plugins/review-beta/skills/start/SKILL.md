...
| Two aspects agree on the same location    | Accept **only if their probe rows ran different binaries, or none**. Two forks reading one tool's stdout is one observation, not two, and still owes a refutation attempt. If an aspect did not relay which binary produced the row, you cannot establish independence — treat it as one observation |
