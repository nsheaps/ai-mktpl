#!/bin/sh
# probe-correctness.sh — programmatic pre-filter for the 'correctness' review family.
#
# Runs the type checkers, vet-class analysers and diff-coverage tools that decide the correctness family, and declares everything that only a CI-scale run can decide.
#
# Usage:
#   probe-correctness.sh [--base REF] [--list] [PATH]
#
...
usage() {
  echo "usage: probe-correctness.sh [--base REF] [--list] [PATH]" >&2
  exit 2
}
...
[ -d "$ROOT" ] || {
  echo "probe-correctness.sh: no such directory: $ROOT" >&2
  exit 2
}
cd "$ROOT" || exit 2
...
TOOLS=$(
  cat <<'TOOLTABLE'
correctness-null-absence,correctness-type-safety-escapes,correctness-control-flow-completeness	tsc	blocker	tsc --noEmit 2>&1 | grep -E 'error TS' || true
correctness-null-absence,correctness-type-safety-escapes	mypy	blocker	mypy --strict --warn-unused-ignores . 2>&1 | grep -E ': error:' || true
correctness-null-absence,correctness-resource-lifecycle,correctness-concurrency,correctness-control-flow-completeness,correctness-error-propagation,correctness-time-clock	go	blocker	go vet ./... 2>&1 | grep -E '^[^ ]+\.go:[0-9]+' || true
correctness-error-propagation	errcheck	blocker	errcheck ./... 2>&1 | grep -E '\.go:[0-9]+' || true
correctness-error-propagation,correctness-null-absence,correctness-control-flow-completeness	eslint	blocker	eslint . -f unix 2>/dev/null | grep -E ':[0-9]+:[0-9]+' || true
correctness-numeric-precision,correctness-retry-timeout,correctness-idempotency,correctness-time-clock,correctness-termination-bounds,correctness-algorithmic-cost,correctness-partial-failure	semgrep	blocker	{ [ -f .semgrep.yml ] || [ -f .semgrep.yaml ] || [ -d .semgrep ]; } && semgrep scan --quiet --metrics=off --vim . 2>/dev/null | grep -E '^[^ ]+:[0-9]+' || true
correctness-test-coverage	diff-cover	major	[ -f coverage.xml ] && diff-cover coverage.xml --compare-branch="$BASE" --fail-under=0 2>/dev/null | grep -E '^[^ ]+\.[a-z]+ ' || true
TOOLTABLE
)
...
printf "# deferred=%s reason=%s\n" \
  "correctness-boundary-conditions" "fuzz/property corpora run in CI, not in a review turn" \
  "correctness-concurrency" "race detector and TSan need a full suite run" \
  "correctness-idempotency" "replay harness needs testcontainers" \
  "correctness-transactional-integrity" "isolationtester specs need a live database" \
  "correctness-test-quality" "mutation testing is a CI-scale job" >>"$TMP/meta"
