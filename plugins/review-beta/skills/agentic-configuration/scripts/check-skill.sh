...
# ---------------------------------------------------------------------------
# Flags are rotated out of "$@" and targets are rotated to the back, so the
# paths stay in the positional parameters. A `targets="$targets $1"` string
# would be word-split on the way back out, and any path containing a space
# would be checked as two nonexistent paths — POSIX sh has no arrays, and the
# positional list is the only list it does have.
argc=$#
ntargets=0
while [ "$argc" -gt 0 ]; do
  arg=$1
