#!/bin/sh
# probe-docs.sh — programmatic pre-filter for the 'docs' review family.
#
# Runs the documentation toolchain -- link checker, markdown linter, prose linter, spell checker, doctest runners -- that decides most of the docs family outright.
#
# Usage:
#   probe-docs.sh [--base REF] [--list] [PATH]
#
...
usage() {
	echo "usage: probe-docs.sh [--base REF] [--list] [PATH]" >&2
	exit 2
}
...
[ -d "$ROOT" ] || {
	echo "probe-docs.sh: no such directory: $ROOT" >&2
	exit 2
}
cd "$ROOT" || exit 2
...
TOOLS=$(
	cat <<'TOOLTABLE'
docs-link-graph-integrity,docs-url-stability-and-redirects	lychee	major	lychee --no-progress --offline . 2>&1 | grep -E '^\[ERROR\]' || true
docs-presentation,docs-entry-point-completeness,docs-accessibility	markdownlint-cli2	nit	markdownlint-cli2 '**/*.md' 2>&1 | grep -E '^[^ ]+\.md:[0-9]+' || true
docs-terminology-consistency,docs-claim-substantiation,docs-presentation	vale	major	vale --output line . 2>/dev/null | grep -E ':[0-9]+:[0-9]+' || true
docs-terminology-consistency	cspell	nit	cspell '**/*.md' --no-progress --no-summary 2>/dev/null | grep -E ':[0-9]+:[0-9]+' || true
docs-presentation	prettier	nit	prettier --check '**/*.md' 2>&1 | grep -E '^\[warn\] ' || true
docs-example-correctness	pytest	major	[ -d docs ] && pytest --doctest-modules --doctest-glob='*.md' -q docs 2>/dev/null | grep -E '^(FAILED|ERROR)' || true
docs-example-correctness	cargo	major	[ -f Cargo.toml ] && cargo test --doc 2>&1 | grep -E '^(test .* FAILED|error)' || true
docs-api-reference-coverage	interrogate	major	interrogate -v . 2>/dev/null | grep -E '^ *[0-9.]+% ' || true
docs-link-graph-integrity	mkdocs	major	[ -f mkdocs.yml ] && mkdocs build --strict 2>&1 | grep -E 'WARNING|ERROR' || true
TOOLTABLE
)
...
printf "# deferred=%s reason=%s\n" \
	"docs-getting-started-executable" "quickstart execution needs a clean container" \
	"docs-translation-sync" "source-digest comparison needs the localized corpus" >>"$TMP/meta"
