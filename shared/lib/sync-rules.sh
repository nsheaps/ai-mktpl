#!/usr/bin/env bash
# sync-rules.sh — Shared library for rules-bundle plugins
#
# Syncs a plugin's rules/ directory into .claude/rules/ via symlink.
# Supports cross-repo plugin enablement and user-scope syncing.
#
# Required variables (set before sourcing):
#   PLUGIN_NAME       — plugin identifier (e.g. "common-sense")
#   LINK_NAME         — symlink directory name (usually same as PLUGIN_NAME)
#   PLUGIN_RULES_DIR  — absolute path to the plugin's rules/ directory
#   CLAUDE_PLUGIN_ROOT — set by Claude Code
#
# Required libraries (source before this):
...
if [ "${_SYNC_RULES_LOADED:-}" = "true" ]; then return 0; fi
_SYNC_RULES_LOADED="true"
...
setup_symlink() {
  ...
  local ai_mktpl_pattern="ai-mktpl/plugins/${PLUGIN_NAME}/"
  for entry in "$target_dir"/*; do
    ...
    if [ -L "$entry" ]; then
      ...
      if [[ "$resolved" == *"${ai_mktpl_pattern}"* ]]; then
        echo "${PLUGIN_NAME}: removing stale symlink ${entry} -> ${resolved}" >&2
        rm -f "$entry"
      fi
    fi
  done
  ...
  if [ -L "$link_path" ]; then
    ...
    echo "${PLUGIN_NAME}: WARNING: ${link_path} is a real directory, not replacing" >&2
    return 0
  fi
...
add_plugin_to_repo() {
  ...
  if [ -f "$target_file" ]; then
    local current
    current="$(jq -r ".enabledPlugins[\"${PLUGIN_NAME}@nsheaps-claude-plugins\"] // empty" "$target_file" 2>/dev/null || true)"
  ...
  # SETTINGS_FILE is intentionally global — safe_write_settings reads it
  SETTINGS_FILE="$target_file"
  safe_write_settings ".enabledPlugins[\"${PLUGIN_NAME}@nsheaps-claude-plugins\"] = true"
  echo "${PLUGIN_NAME}: enabled plugin in ${target_file}" >&2
}
...
sync_rules_run() {
  local ALSO_SYNC_TO_USER ALSO_ADD_TO_REPOS SYNC_SETTINGS_TARGET
  ...
  local PROJECT_RULES_DIR="${CLAUDE_PROJECT_DIR:-.}/.claude/rules"
  local USER_RULES_DIR="${HOME}/.claude/rules"
  ...
  if [ -n "$ALSO_ADD_TO_REPOS" ] && command -v jq &>/dev/null; then
    ...
    for candidate in "$project_parent"/*/; do
      ...
      local candidate_real project_real
